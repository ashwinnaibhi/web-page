...
#         return redirect(url_for('login_page'))
#
#     # Default student_id = logged-in student
#     student_id = session.get('user_id')
#
#     all_students = fdb.list_all_students()  # Get all students
#
#     # If parent, allow selecting a student via query param
#     if session.get('role') == 'parent':
#         student_id = request.args.get("student_id") or (all_students[0]['USN'] if all_students else None)
#         if not student_id:
#             return "<h3>No students found for this parent!</h3>"
#
#     # Get student info
...
def get_attendance_week(date_str):
    ...
    if session.get('role') == 'parent':
        student_id = request.args.get('student_id', student_id)
...
def update_attendance():
    ...
    success = fdb.update_attendance(data['student_id'], data['date'], data['time'], data['status'])
    if success:
...
def update_student_attendance():
    ...
    if request.method == "POST":
        ...
        if record_id:
            fdb.update_attendance(student_id=student_id, date=date, time=time_slot, status=status, subject=subject)
            return jsonify({"success": True, "message": "Updated", "sheet_row": 0})
        else:
            fdb.create_attendance(student_id, date, time_slot, subject, status, timestamp)
            return jsonify({"success": True, "message": "Row created", "sheet_row": 0})
    ...
    print(f"Time Slots in Routes {time_slots}")
    if not time_slots:
    ...
    if not students:
        flash(f"No students enrolled for subject {subject}.", "warning")
        records = []
    else:
